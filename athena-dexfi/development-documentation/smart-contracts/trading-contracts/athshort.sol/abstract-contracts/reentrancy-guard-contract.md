---
description: >-
  ReentrancyGuard abstract contract is built for preventing manipulated
  re-enterancy attack.
---

# Reentrancy Guard Contract

<pre class="language-solidity" data-overflow="wrap"><code class="lang-solidity"><strong>Contract module that helps prevent reentrant calls to a function.
</strong>Inheriting from `ReentrancyGuard` will make the {nonReentrant} modifier available, which can be applied to functions to make sure there are no nested (reentrant) calls to them.
...
abstract contract ReentrancyGuard {
    ...
    function internalNonReentant() internal view {
        // On the first call to nonReentrant, _notEntered will be true
        require(_status != _ENTERED, "err msg");
    }
}
```
</code></pre>
